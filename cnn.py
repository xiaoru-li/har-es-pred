...
import sys
sys.stdout = open('cnn-' + sys.argv[1] + '-' + sys.argv[2] + '-' + str(int(time.time())) + '.txt', 'w+')
...
dataset = sys.argv[1]
if dataset == "opp":
    path = os.path.join('opportunity.h5')
elif dataset =="dap":
    path = os.path.join('daphnet.h5')
elif dataset =="pa2":
    path = os.path.join('pamap2.h5')
elif dataset =="sph":
    path = os.path.join('sphere.h5')
else:
...
learning_rate = 0.0001 if sys.argv[1] == 'pa2' else 0.0005
...
with tf.Session() as session:
    ...
    for epoch in range(training_epochs):
        ...
        t_loss = np.mean(cost_history)
        t_accuracy = session.run(accuracy, feed_dict={X: train_x, Y: train_y, dropout_1: 1-0.1, dropout_2: 1-0.25, dropout_3: 1-0.5})
        ...
        print "Epoch: ",epoch," Training Loss: ",t_loss," Training Accuracy: ",t_accuracy," f1: ",f1_score_m," Validation Accuracy: ",val_accuracy
